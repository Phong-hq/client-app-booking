<template>
  <div class="staff-list"  >
      <el-drawer v-model="drawer" :show-close="false" :direction="'btt'">
        ...
        <h4 class="text-center font-bold-ct text-18px text-darkest1" :id="titleId" :class="titleClass">Select Staff</h4>
        </template>
        <div  class="w-full px-4">
            <div class="my-4 relative">
                <el-input class="search" v-model="search" placeholder="Search services"  @input="searchStaff()"/>
            ...
            <div class="list">
                <div class="" v-for="item in staffList" :key="item.id">
                    <div class="item mb-4 px-2  bg-white w-full flex justify-between items-center" v-if="item.search">
                        <div class="flex items-center">
                            ...
                            <p class="text-16px font-medium-ct">{{item.name}}</p>
                        </div>
                        <div class="flex items-center">
                            <el-button class="book-btn text-16px text-white" color="#5F2EEA" >Book</el-button>
                        </div>
                    </div>
                </div>
            </div>
...
<script setup>
    ...
    const search = ref('')
    const staffList = ref([
        {id: 0, search: true, name: 'Any staff available',},
        {id: 1, search: true, name: 'Cloe Le',},
        {id: 2, search: true, name: 'Daisy Nguyen',},
        {id: 3, search: true, name: 'Hannah',},
        {id: 4, search: true, name: 'Daniel Tran',},
        {id: 5, search: true, name: 'Helen Pham',},
        {id: 6, search: true, name: 'Helen Pham',},
        {id: 7, search: true, name: 'Daisy Nguyen',},
        {id: 8, search: true, name: 'Cloe Le',},
        {id: 9, search: true, name: 'Daniel Tran',},
        {id: 10, search: true, name: 'Daisy Nguyen',},
        {id: 11, search: true, name: 'Daisy Nguyen',},
        {id: 12, search: true, name: 'Hannah',},
    ])
    ...
    const closeDrawer = () => {
        drawer.value = false;
        console.log('asdsdsdas');
    }

    const searchStaff = () => {
        staffList.value.map(item => item.search = true)
        for(let i = 0 ; i < staffList.value.length ; i++){
            let nameValue =  nonAccentVietnamese(staffList.value[i].name)
            let searchValue = nonAccentVietnamese(search.value)
            staffList.value[i].search = nameValue.includes(searchValue)
        }
    }
